...
**Decisions (by reasoning):** (1) a saved chat can be opened only by its owner, not by other customers, staff or the admin (others get 404). *Corrected 2026-09-24, v2.20.1:* the audit log separately keeps bounded excerpts of every chat (question: first 200 characters; answer: first 500), which the branch manager and admin can review, so "private" means the saved conversation, not every trace of it; (2) the AI still sees only the current question, as today, because earlier turns would slow answers (B2) and change behaviour, so this is history for people, not memory for the AI; (3) trimmed: no "create session" address (the first message creates one), the latest 50 chats with no paging, and no context-turns setting.
