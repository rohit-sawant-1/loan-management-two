...
export default function DocumentChecklist({ applicationId, data, onChange }) {
  ...
  const [batchMode, setBatchMode] = useState(false);  // Piece 35: the several-at-once form is showing
  const [batchDocs, setBatchDocs] = useState([]);     // Piece 35: uploaded together, details to check
  ...
  return (
    <div>
      ...
      {/* Piece 35: the documents just uploaded together, to check and confirm in one go. */}
      {batchDocs.length > 0 && (
        <BatchReview docs={batchDocs} canEdit={!isAdmin} onChanged={onChange} onDone={() => setBatchDocs([])} />
      )}
        ...
        batchMode ? (
          ...
          <>
            <RealUploadForm applicationId={applicationId} onChange={onChange} onUploaded={setReviewing}
              setError={setError} />
            <Button type="button" size="sm" variant="ghost" icon="plus" onClick={() => setBatchMode(true)}>
              Upload several at once (up to {MAX_FILES_AT_ONCE})
            </Button>
          </>
        )
      ) : (
